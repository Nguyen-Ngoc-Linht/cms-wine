...
import { fetchReportStatisticStation, exportReportStatisticStation } from '@/api/report.js'
...
const getList = async () => {
  ...
  try {
    ...
    const { data, total } = await fetchReportStatisticStation(param)
...
const handleExport = async () => {
  ...
  try {
    const param = getParamFilter()
    const { data } = await exportReportStatisticStation(param)
    if (data) {
