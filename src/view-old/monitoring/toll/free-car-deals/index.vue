<template>
  <div>
    <div class="flex flex-wrap gap-px-8 items-center justify-start paddingX-24 mt-5">
      <el-select
        :placeholder="t('monitoring.toll.exemptVehicleType')"
        v-model="filter.freeType"
        clearable
        multiple
        filterable
        collapse-tags
    ...
    <div class="paddingX-24 mb-4">
      <div class="px-3 py-3 bg-white">
        ...
        >
          ...
          <el-table-column :label="t('monitoring.toll.freeType')">
            <template #default="{ row }">
              <span>{{ row.free_type }}</span>
            </template>
          </el-table-column>
...
const listFreeType = ref([])
const listVehicleType = ref([])

const listFreeVehicle = ref([])
const infoFreeVehicle = ref({})
...
const setDataDefault = async () => {
  ...
  const list = await apiGetFreeTypeAll()
  if (list.status === 200) {
    listFreeType.value = list.data
  }
  const listVehicle = await apiGetTypeVehicleAll()
  if (listVehicle.status === 200) {
    listVehicleType.value = listVehicle.data.sort((a, b) => a.id - b.id)
  }
}
const getList = async () => {
  ...
  if (rs.status === 200) {
    listFreeVehicle.value = rs.data.data
...
const handleUpdateSuspect = async data => {
  try {
    ...
    if (rs.status === 200) {
      console.log(listFreeVehicle.value)
    }
  } catch (e) {
    console.log(e)
  }
