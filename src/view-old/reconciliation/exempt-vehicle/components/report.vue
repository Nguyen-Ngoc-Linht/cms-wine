    ...
    <div class="paddingX-24">
      ...
      >
        ...
        >
          ...
          <el-table-column :label="t('dateTime.day')">
          ...
          <el-table-column :label="t('reconciliation.freeTransNationwide')">
            <template #default="{ row }">
              <span>{{ formatNumber(row.freeNation, '.') }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="t('reconciliation.freeTransSingle')">
            <template #default="{ row }">
              <span>{{ formatNumber(row.freeSingle, '.') }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="t('reconciliation.freeTransHard')">
            <template #default="{ row }">
              <span>{{ formatNumber(row.freeHardKey, '.') }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="t('reconciliation.freeTransConvoys')">
            <template #default="{ row }">
              <span>{{ formatNumber(row.freeGroup, '.') }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="t('reconciliation.freeTransETC')">
            <template #default="{ row }">
              <span>{{ formatNumber(row.freeEtc, '.') }}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import { apiGetVehicleReconcReport, apiExportVehicleReconcReport } from '@/api/reconciliation'
...
const getList = () => {
  ...
  apiGetVehicleReconcReport(data)
    .then(res => {
...
const handleExport = isExcel => {
  ...
  apiExportVehicleReconcReport(data)
    .then(res => {
