...
XPMobileSDKTunnel.library.CHAP = {
	/**
	...
	initialize: function () {
		...
		XPMobileSDKTunnel.library.Connection.addObserver(this);
	},
	...
	cleanUp: function() {
		...
		if (!XPMobileSDKTunnel.library.Connection.connectionId) return;
		...
		if(this.challenges.length < this.minChallenges) {
			...
			XPMobileSDKTunnel.requestChallenges(params, () => {
	...
	add: function (challenges) {
		...
		if(typeof challenges == 'string') {
			var challenge = new XPMobileSDKTunnel.library.Challenge(challenges);
			this.challenges.push(challenge);
		}
		else if(typeof challenges == 'object' && challenges.length > 0) {
			for(var i = 0; i < challenges.length; i++) {
				var challenge = new XPMobileSDKTunnel.library.Challenge(challenges[i]);
				this.challenges.push(challenge);
			}
		}
		this.start();
	...
	connectionRequestSucceeded: function(request, response) {
	    if (XPMobileSDKTunnel.library.Connection.CHAPSupported == 'Yes'
            && response
            && response.parameters
            && response.parameters.Challenge
            && XPMobileSDKTunnel.library.Connection.connectionId) {
	...
	destroy: function() {
		...
		this.challenges = [];
		this.unHaltSystem();
		XPMobileSDKTunnel.library.SecureString.sharedKey = null;
	}
};