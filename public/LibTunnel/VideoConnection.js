 ...
 */
XPMobileSDKTunnel.library.VideoConnectionSignal = {
 ...
 */
XPMobileSDKTunnel.library.VideoConnectionState = {
 ...
 */
XPMobileSDKTunnel.library.VideoConnectionStream = {
 ...
 */
XPMobileSDKTunnel.library.VideoConnectionObserverInterface = {
 ...
 * Do not create instances of that class directly. Instead call XPMobileSDKTunnel.requestStream to first prepare the connection to the
 ...
 */
XPMobileSDKTunnel.library.VideoConnection = function (videoId, connectionRequest, callbacks) {

    XPMobileSDKTunnel.library.VideoConnection.instances.push(this);
    ...
    var observers = [];
    var state = XPMobileSDKTunnel.library.VideoConnectionState.notOpened;
    ...
    this.open = function () {
        switch (state) {
            case XPMobileSDKTunnel.library.VideoConnectionState.notOpened:
                setState(XPMobileSDKTunnel.library.VideoConnectionState.running);
                if (window.Worker && XPMobileSDKSettingsTunnel.supportsMultiThreaded) {
                    ...
                    if (self.isPush) {
                        ...
                        self.communication = new XPMobileSDKTunnel.library.PushConnection(connectionURL + '/' + self.videoId + '/', { 'signalType': self.signalType});
                    }
                    else {
                        console.info('Opening AJAX video connection ' + self.videoId);
                        self.communication = new XPMobileSDKTunnel.library.PullConnection(connectionURL + '/' + self.videoId + '/', { 'signalType': self.signalType});
                    }
                ...
                if (state == XPMobileSDKTunnel.library.VideoConnectionState.running) {
                    resetNoVideoTimeout();
                }

                break;
            case XPMobileSDKTunnel.library.VideoConnectionState.running:
                if (self.isReusable) {
                ...
                resetNoVideoTimeout();
                break;
            case XPMobileSDKTunnel.library.VideoConnectionState.closed:
    ...
    var resetNoVideoTimeout = function () {
        ...
        }, XPMobileSDKSettingsTunnel.NoVideoTimeout);
    };
    ...
    this.restart = function () {
        ...
        if (state == XPMobileSDKTunnel.library.VideoConnectionState.closed) return;

        console.warn('Restarting video connection ' + self.videoId + ' for camera ' + self.cameraId);
        setState(XPMobileSDKTunnel.library.VideoConnectionState.closed);
    ...
    this.close = function () {
        ...
        if (state == XPMobileSDKTunnel.library.VideoConnectionState.closed ||
            state == XPMobileSDKTunnel.library.VideoConnectionState.closing) return;

        setState(XPMobileSDKTunnel.library.VideoConnectionState.closing);


        if (self.worker && XPMobileSDKSettingsTunnel.supportsMultiThreaded) {
    ...
    this.destroy = function () {
        ...
        if (XPMobileSDKTunnel.library.VideoConnection.indexOf(this) != -1) {
            XPMobileSDKTunnel.library.VideoConnection.splice(XPMobileSDKTunnel.library.VideoConnection.indexOf(this), 1);
        }
    };
    ...
    function notifyObservers(item) {
        ...
        if (item.stream && item.stream.error) {
            switch (item.stream.error) {
                case XPMobileSDKTunnel.library.ItemHeaderParser.Error.NonFatal:
                    // Fallback to Transcoded streaming
                    self.request.parameters.StreamType = 'Transcoded';
                    self.restart();
                    break;
                case XPMobileSDKTunnel.library.ItemHeaderParser.Error.Fatal:
    ...
    function Channel(parameters) {
        ...
        if (!channels.length) channels.push(parseURL(XPMobileSDKSettingsTunnel.MobileServerURL + XPMobileSDKSettingsTunnel.videoChanel));
    ...
    function onError(request) {
        ...
        if (state == XPMobileSDKTunnel.library.VideoConnectionState.closed || XPMobileSDKTunnel.library.Connection.connectionId == null) {
            return;
        }
        var timeout = 0;
...
XPMobileSDKTunnel.library.VideoConnection.instances = [];
