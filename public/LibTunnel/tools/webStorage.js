 ...
 * This library provides proxies for these storages, called XPMobileSDKTunnel.localStorage nad XPMobileSDKTunnel.sessionStorage. 
 ...
 *
 * Example:
 *		XPMobileSDKTunnel.sessionStorage.setItem('myKey', true);
 *		XPMobileSDKTunnel.sessionStorage.getItem('myKey') === true;
 *
 ...
 *
 * Example:
 *		XPMobileSDKTunnel.sessionStorage.setItem('myKey', 'myValue', 1000 * 60);
 *		// after a minute
 *		XPMobileSDKTunnel.sessionStorage.getItem('myKey') === null;
 *
 * @author rvp
 */
(function () {
		...
		XPMobileSDKTunnel[storages[i]] = {
			...
			setItem: function (key, value, expiration) {
				...
				if (expiration) {
					...
					this.storage.setItem(key + 'Tunnel', 'expiration::' + JSON.stringify(obj));
					return ;
				}

				if (typeof value == 'boolean') {
					this.storage.setItem(key + 'Tunnel', 'boolean::' + value);
					return;
				}

				if (typeof value == 'number') {
					this.storage.setItem(key + 'Tunnel', 'number::' + value);
					return;
				}

				if (typeof value == 'object') {
					this.storage.setItem(key + 'Tunnel', 'object::' + JSON.stringify(value));
					return;
				}

				this.storage.setItem(key + 'Tunnel', value);
			},

			getItem: function (key) {

				var value = this.storage.getItem(key + 'Tunnel');
				...
				if (value.indexOf('expiration::') == 0) {
					...
					if (new Date().getTime() > obj.expiration) {
						this.storage.removeItem(key + 'Tunnel');
						return null;
					} 
					return obj.value;
				}
