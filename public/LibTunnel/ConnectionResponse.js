 ...
 */
XPMobileSDKTunnel.library.ConnectionResponse = function (xmlString) {
	...
	if (XMLNodeTextContent(TypeNode) == 'Processing') {
		...
		if (XMLNodeTextContent(ResultNode) != 'OK') {
			...
			self.error = {
				code: self.errorCode || XPMobileSDKTunnel.library.ConnectionError.Unknown,
	...
	/**
	 * Gets an error string by error code as defined in XPMobileSDKTunnel.library.ConnectionError
	 */
	function getError(errorCode) {
		var result;
		Object.keys(XPMobileSDKTunnel.library.ConnectionError).forEach(function(key) { 
				if (XPMobileSDKTunnel.library.ConnectionError[key] == errorCode) { 
					result = key; 
				} 
			});
		return result;
