XPMobileSDKTunnel.library.PullConnectionObserverInterface = {
...
XPMobileSDKTunnel.library.PullConnection = function(videoURL, options) {
	...
	var self = this;
	self.videoConnectionState = XPMobileSDKTunnel.library.VideoConnectionState.running;
	...
	function sendRequest(timeout){
		self.nextFrameTimeout = setTimeout(function() {
			...
			if (self.videoConnectionState != XPMobileSDKTunnel.library.VideoConnectionState.closed) {
				self.ajaxRequest = new XPMobileSDKTunnel.library.Ajax.Request(videoURL, frameRequestParams);
			}
		}, timeout || 1);
	};
	...
	function notClosed() {
		return self.videoConnectionState != XPMobileSDKTunnel.library.VideoConnectionState.closed;
	};
	...
	function onAjaxLoading (ajaxRequest) {
		if(notSarted()) {	
			...
			var timeout = XPMobileSDKSettingsTunnel.videoConnectionTimeout;
			self.ajaxRequestTimeout = setTimeout(function () {
				...
				ajaxRequest.abort();
				XPMobileSDKTunnel.library.Ajax.activeRequestCount--;
	...
	function onAjaxSuccess (ajaxRequest) {
		...
		if(notClosed()){
			...
			try {
				...
				var currentData = new XPMobileSDKTunnel.library.VideoHeaderParser(res);
				...
				if (self.videoConnectionState == XPMobileSDKTunnel.library.VideoConnectionState.running) {
					...
					else if (!currentData.dataSize && self.signalType == XPMobileSDKTunnel.interfaces.VideoConnectionSignal.live) {
