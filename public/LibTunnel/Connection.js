 ...
 */
XPMobileSDKTunnel.library.ConnectionStates = {
 ...
 */
XPMobileSDKTunnel.library.ConnectionObserverInterface = {
...
ConnectionTunnel = function() {
	 ...
	 */
	this.state = XPMobileSDKTunnel.library.ConnectionStates.idle; 
	 ...
	 * 
	 * @method initialize
	 * @param storage: optional, the storage used to store server features in, and to initialize them from (for example XPMobileSDKTunnel.localStorage, XPMobileSDKTunnel.sessionStorage, or any object implementing their methods). 
	...
	this.initialize = function (storage) {
		...
		if (self.storage = storage) {
			XPMobileSDKTunnel.features = self.storage.getItem('features');
		...
		self.server = XPMobileSDKSettingsTunnel.MobileServerURL || window.location.origin;
		self.dh = new XPMobileSDKTunnel.library.DiffieHellman();
	};
	...
	this.Connect = function (params, successCallback, failCallback) {
		...
		setState(XPMobileSDKTunnel.library.ConnectionStates.connecting);
		params.ProcessingMessage = 'No';
	    return self.sendCommand('Connect', params, { successCallback: successCallback }, connectCallbackTunnel, failCallback);
	};
	
	var connectCallbackTunnel = function (connectionRequest) {
		requestFinished(connectionRequest);
		...
		if (!connectionResponse || connectionResponse.isError) {
			...
			if (self.storage) {
                ...
                if (typeof self.storage.getItem('directStreamingClient') == 'boolean') {
                    ...
                    self.directStreamingClient = !!XPMobileSDKSettingsTunnel.DirectStreaming;
            ...
            if (connectionResponse.outputParameters.CHAPSupported && connectionResponse.outputParameters.CHAPSupported == "Yes") {
				...
				XPMobileSDKTunnel.library.CHAP.sharedKey = self.dh && self.dh.getSharedKey();
			}
	...
	this.Login = function (params, successCallback, failCallback) {
	    ...
	    setState(XPMobileSDKTunnel.library.ConnectionStates.loggingIn);
	    return self.sendCommand('LogIn', params, { successCallback: successCallback }, loginCallbackTunnel, failCallback);
	};
	
	var loginCallbackTunnel = function (connectionRequest) {
		requestFinished(connectionRequest);
		var connectionResponse = connectionRequest.response;
		if (!connectionResponse || connectionResponse.isError) {
			if (connectionResponse && connectionResponse.error.code == XPMobileSDKTunnel.library.ConnectionError.SecondStepAuthenticationRequired) {
			...
			proceedWithLoginTunnel(connectionResponse, connectionRequest.options.successCallback);
		}
	};
	
	var proceedWithLoginTunnel = function (connectionResponse, successCallback) {
        
        var oldServerVersion = XPMobileSDKTunnel.features && XPMobileSDKTunnel.features.ServerVersion;
		...
		setState(XPMobileSDKTunnel.library.ConnectionStates.working);
		callMethodOnObservers('connectionDidLogIn', connectionResponse.outputParameters);
        successCallback && successCallback();

		if (oldServerVersion && oldServerVersion != XPMobileSDKTunnel.features.ServerVersion) {
	...
	this.verifyCode = function (code) {
		...
		return self.sendCommand('VerifySecondStepAuthenticationPin', params, null, verifyCodeCallbackTunnel);
	};
	
	var verifyCodeCallbackTunnel = function (connectionRequest) {
		requestFinished(connectionRequest);
		var connectionResponse = connectionRequest.response;
		if (!connectionResponse || connectionResponse.isError) {
			if (connectionResponse && connectionResponse.error.code == XPMobileSDKTunnel.library.ConnectionError.SecondStepAuthenticationCodeError) {
			...
			proceedWithLoginTunnel(connectionResponse);
		}
	};
	...
	this.Disconnect = function (params, successCallback, failCallback) {
		...
		cancelLiveMessage();
		setState(XPMobileSDKTunnel.library.ConnectionStates.idle);

		XPMobileSDKTunnel.library.VideoConnectionPool.clear();
		var params = params || {};
		var connectionRequest = self.sendCommand('Disconnect', params, { successCallback: successCallback }, logOutCallbackTunnel, failCallback);
	 ...
	 */
	var logOutCallbackTunnel = function(connectionRequest) {
		requestFinished(connectionRequest);
	...
	this.getViews = function (viewId, successCallback, failCallback) {
	    return self.sendCommand('GetViews', { ViewId: viewId }, { successCallback: successCallback, ViewId: viewId }, getViewsCallbackTunnel, failCallback);
	};
	
	var getViewsCallbackTunnel = function (connectionRequest) {		
	...
	this.requestStream = function (cameraId, size, options, successCallback, failCallback) {
		...
		var params = {
			...
			SignalType: options.signal === XPMobileSDKTunnel.interfaces.VideoConnectionSignal.playback ? 'Playback' : 'Live',
			MethodType: 'Push', 
			...
			RequestSize: 'Yes',
            StreamType: options.streamType == XPMobileSDKTunnel.library.VideoConnectionStream.FragmentedMP4 ? 'FragmentedMP4' : 'Transcoded'
		};
		...
		if (XPMobileSDKTunnel.features.SupportNoScaledImages) {
			params.ResizeAvailable = 'Yes';
		}
		
		if (XPMobileSDKTunnel.features.MultiCameraPlayback && options.playbackControllerId) {
		...
		var options = {
			successCallback: successCallback,
			cameraId: cameraId,
			signal: options.signal === XPMobileSDKTunnel.interfaces.VideoConnectionSignal.playback ? 'Playback' : 'Live',
	...
	var requestStreamCallback = function (connectionRequest) {
		callbackAfterRequest(connectionRequest, 'Error starting stream for camera ' + connectionRequest.options.cameraId, function () {
			...
			let videoConnection = new VideoStreamTunnel(videoId, connectionRequest);
			if (connectionRequest.options.reuseConnection) {
				XPMobileSDKTunnel.library.VideoConnectionPool.addCamera(connectionRequest.options.cameraId, videoConnection);
			}
    ...
    */
	...
	var closeVideoConnection = function (videoConnection) {
		...
		videoConnection.isReusable && XPMobileSDKTunnel.library.VideoConnectionPool.removeCamera(videoConnection.cameraId, videoConnection.videoId);
	...
	this.requestAudioStream = function (microphoneId, options, successCallback, failCallback) {
	    ...
	    else if (XPMobileSDKSettingsTunnel.AudioCompressionLevel) {
	        params.ComprLevel = XPMobileSDKSettingsTunnel.AudioCompressionLevel;
	    }
    ...
    this.toggleDirectStreaming = function (enabled) {
        ...
        XPMobileSDKTunnel.library.VideoConnection.instances.forEach(function (videoConnection) {
            videoConnection.getState() == XPMobileSDKTunnel.library.VideoConnectionState.running && videoConnection.close();
        });
    };
	...
	var restartCameras = function() {
		...
		XPMobileSDKTunnel.library.VideoConnection.instances.forEach(function(videoConnection) {
		    videoConnection.getState() == XPMobileSDKTunnel.library.VideoConnectionState.running && videoConnection.restart();
		});
	};
	...
	var changeStreamCallback = function (connectionRequest) {
		callbackAfterRequest(connectionRequest, 'Error changing stream.', function () {
			if (XPMobileSDKTunnel.features.SupportTimeBetweenFrames) {
	...
	var fps = new function () {
		...
		var change = function (fps, successCallback, failCallback) {
			...
			for (var i = 0, camera; camera = XPMobileSDKTunnel.library.VideoConnectionPool.cameras[i]; i++) {
	...
	var createPlaybackControllerCallback = function(connectionRequest) {
		...
		var connectionResponse = connectionRequest.response;
		var videoConnection = new XPMobileSDKTunnel.library.VideoConnection(
	...
	this.registerForNotifications = function (setting, successCallback, failCallback) {
	    ...
	    var params = {
	        Settings: setting, 
	        DeviceName: XPMobileSDKTunnel.library.Connection.dh.encodeString(deviceName),
	        DeviceId: XPMobileSDKTunnel.library.Connection.connectionId
	    };
	...
	this.sendCommand = function (commandName, requestParams, options, successCallback, failCallback) {
		...
		if (XPMobileSDKSettingsTunnel.supportsCHAP && self.SecurityEnabled == 'Yes' && self.CHAPSupported == 'Yes') {
	        var challenge = XPMobileSDKTunnel.library.CHAP.calculate();
			...
			} else if (XPMobileSDKTunnel.library.Connection.state == XPMobileSDKTunnel.library.ConnectionStates.working) {
	    ...
	    var connectionRequest = new XPMobileSDKTunnel.library.ConnectionRequest(commandName, getNextSequenceID(), requestParams, options, successCallback);
	...
	var getFeatures = function (features) {
		...
		for (i in features) {
			switch(i) {
				...
				case 'ServerVersion':
					...
					break;
				case 'ServerType':
					XPMobileSDKSettingsTunnel.ServerType = features[i];
					break;
				case 'ServerDescription':
					XPMobileSDKSettingsTunnel.ServerDescription = features[i];
					break;
				default:
		...
		XPMobileSDKTunnel.features = data;

	};
	
	/**
	 * Adds the status of the Direct streaming to the XPMobileSDKTunnel.features object.
	 * 
	 *  XPMobileSDKTunnel.features.DirectStreaming == 0 - DS not available
	 *  XPMobileSDKTunnel.features.DirectStreaming == 1 - do not enforce
	 *  XPMobileSDKTunnel.features.DirectStreaming == 2 - enforce whenever possible
	 *  XPMobileSDKTunnel.features.DirectStreaming == 3 - enforce for all clients
	 */
	...
	var connectionRequestResponseIsTerminal = function (connectionRequest) {
		...
		if (connectionResponse == undefined
            || connectionResponse.errorCode == XPMobileSDKTunnel.library.ConnectionError.WrongID
            || connectionResponse.errorCode == XPMobileSDKTunnel.library.ConnectionError.ChallengesLimitReached
	...
	var lostConnection = function () {
		if (self.state != XPMobileSDKTunnel.library.ConnectionStates.lostConnection) {
			setState(XPMobileSDKTunnel.library.ConnectionStates.lostConnection);
			self.connectionId = null;
			XPMobileSDKTunnel.library.VideoConnectionPool.clear();
			self.destroy();
 ...
 */ 
XPMobileSDKTunnel.library.ConnectionError = {
	NotImplemented: 1,
